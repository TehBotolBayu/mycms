...
export function BlogPostCardH ({ name, img, title, desc, tags, date }: BlogPostCardProps) {
  return (
    ...
    <div className="relative bg-clip-border rounded-xl bg-white text-gray-700 shadow-lg w-full p-2 md:p-6">
      ...
      <div className="flex">
      <div className="flex items-center relative w-2/5 m-0 overflow-hidden text-gray-700  rounded-r-none bg-clip-border rounded-xl shrink-0 ">
        <img
          src={img}
          alt="card-image"
          className="object-cover object-center w-full md:h-[200px] h-16"
        />
      </div>
      <div className="p-6">
        ...
        <p className="hidden md:block mb-8 font-sans text-base antialiased font-normal leading-relaxed text-gray-700 text-ellipsis overflow-hidden ...">
          {desc}
        </p>
        <div>
        {(tags && tags[0])
